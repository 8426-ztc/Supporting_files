...
def run(args: argparse.Namespace) -> None:
    ...
    previous_state_path = prediction_dir / f"codex_inference_state_epoch02{tag}.json"
    state_path = prediction_dir / f"codex_inference_state_epoch03{tag}.json"
    ledger_path = prediction_dir / f"codex_inference_ledger_epoch03{tag}.jsonl"
    addendum_path = Path(__file__).resolve().with_name("PREREGISTRATION_ADDENDUM_08.md")
    if not previous_state_path.is_file() or not addendum_path.is_file():
        raise ValueError("Execution epoch 3 requires the frozen epoch-2 state and addendum 08")
    ...
    for record in selected:
        ...
        for attempt in range(1, args.max_attempts + 1):
            ...
            try:
                ...
                stderr = (
                    stderr
                    + f"\nORCHESTRATION_TIMEOUT: attempt exceeded {args.timeout_seconds} seconds\n"
                )
